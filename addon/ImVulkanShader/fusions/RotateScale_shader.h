...
\n\
    ...
    sfpvec4 rgba_to = load_rgba_src2(int(rotatedUv.x * (p.w2 - 1)), int((1.f - rotatedUv.y) * (p.h2 - 1)), p.w2, p.cstep2, p.in_format2, p.in_type2); \n\
    sfpvec4 rgba_from = load_rgba(int(rotatedUv.x * (p.w - 1)), int((1.f - rotatedUv.y) * (p.h - 1)), p.w, p.cstep, p.in_format, p.in_type); \n\
\n\
